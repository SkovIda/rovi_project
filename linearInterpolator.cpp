...
/*    int n_xPos = 7; 
    ...
    //FOR THE PICK TASK
    std::vector< Q > QstartConfigList;
    //rw::math< Q > Qstart;
    // Move bottleFrame to the pick area (Position of this in TableFrame is approx.: (0 0.474 0.21)
    /*bottleFrame->setTransform (Transform3D<> (Vector3D<> (bottleFrame->getTransform (state).P () + Vector3D<> (-0.3, 0.948, 0)),
                                            RPY<> (0, 0, 90 * Deg2Rad)),
                            state);*/
    QstartConfigList = getCollisionFreeSolutions(state, graspTargetFrame, bottleFrame, robotUR5, wc);
    //Q start = (Qstart[0], Qstart[1], Qstart[2], Qstart[3], Qstart[4], Qstart[5]);
    Q startConfiguration = QstartConfigList[0];


    //FOR THE PLACE TASK
    std::vector< Q > QendConfigList;
    //rw::math< Q > Qend;
    // Move bottleFrame to the center of the place area (Position of this in TableFrame is approx.: (0.3 -0.474 0.21)
    bottleFrame->setTransform (Transform3D<> (Vector3D<> (bottleFrame->getTransform (state).P () + Vector3D<> (0.3, -0.948, 0)),
                                             RPY<> (0, 0, 90 * Deg2Rad)),
                             state);
    QendConfigList = getCollisionFreeSolutions(state, graspTargetFrame, bottleFrame, robotUR5, wc);
    //Q end = (Qend[0], Qend[1], Qend[2], Qend[3], Qend[4], Qend[5]);
    Q endConfiguration = QendConfigList[0];
    // for(unsigned int i =0; i<Qstart.size(); i++){
    //     robotUR5 -> setQ(Qstart[i], state);
    //     tStatePath.push_back(rw::trajectory::TimedState(time, state));
    //     time += 0.01;         
    //     }
    ...
    TimedStatePath linearQMotion = linInterp (robotUR5, wc->getDefaultState (), startConfiguration, endConfiguration, 20);
    PathLoader::storeTimedStatePath (*wc, linearQMotion, "./visuaLINEARinterpolator.rwplay");
    //std::cout << "Qstart: [" << Qstart << "," << Qend << "]\n";
    //std::cout << Qstart.size();
